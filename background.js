...
chrome.webRequest.onBeforeSendHeaders.addListener(
  function (details) {
    // Only capture from Amazon-related requests, prioritize GraphQL API calls
    if (details.url && (details.url.includes('amazon') || details.url.includes('amazonaws'))) {

      // Prioritize tokens from GraphQL API requests
      const isGraphQLRequest = details.url.includes('appsync-api') || details.url.includes('graphql');

      for (let header of details.requestHeaders) {
        const headerName = header.name.toLowerCase();

        // Capture specific authorization headers - prioritize the right token types
        if (headerName === "authorization" && header.value) {

          // Skip AWS4-HMAC-SHA256 tokens as they're for different APIs
          if (header.value.startsWith("AWS4-HMAC-SHA256")) {
            console.log("Skipping AWS4 token (wrong type for job API)");
            continue;
          }

          // If this is a GraphQL request, prioritize this token even if we have one
          if (isGraphQLRequest || !authToken) {
            // Accept these token formats (in order of preference):
            if (header.value.startsWith("Bearer ") ||
              header.value.startsWith("AQICAH") ||
              header.value.startsWith("AQ") ||
              (header.value.includes("aws") && !header.value.startsWith("AWS4")) ||
              header.value.length > 200) { // Very long tokens are likely session tokens

              // If this is from GraphQL, replace any existing token
              if (isGraphQLRequest && authToken) {
                console.log("🔄 Replacing token with GraphQL API token");
              }

              authToken = header.value;
              console.log("Captured authToken" + (isGraphQLRequest ? " (from GraphQL)" : "") + ":", header.value.substring(0, 50) + "...");
            }
          }
        }
...
async function searchAmazonJobs() {
  ...
  try {
    ...
    // Handle 403 Forbidden - likely wrong token type
    if (res.status === 403) {
      console.log("🔴 403 Forbidden - likely wrong authorization token type");
      if (authToken && authToken.startsWith("AWS4-HMAC-SHA256")) {
        console.log("🔄 Resetting AWS4 token to look for correct job API token");
        authToken = null; // Reset to capture the correct token type
      }
    }
...
async function processApplication(jobSchedule) {
  ...
  try {
    ...
    // 🎯 PROACTIVE INTERVIEW SCHEDULING - Try immediately after application creation
    console.log("🎯 Attempting proactive interview scheduling...");
    setTimeout(() => {
      autoScheduleInterview(applicationId, candidateId, jobId, scheduleId);
    }, 3000); // Give the application 3 seconds to be processed in their system
    ...
    function attemptScheduleInterviewMessage(applicationId, candidateId, jobId, scheduleId, preferredSlots = []) {
      // Enhanced interview scheduling with more comprehensive slot options
      const msg = {
    ...
    // Enhanced function to automatically find and book available interview slots
    async function autoScheduleInterview(applicationId, candidateId, jobId, scheduleId) {
      try {
        console.log(`🎯 Auto-scheduling interview for application ${applicationId}`);

        // 1. Try to get available interview slots from the API
        const slotsResponse = await fetchWithTimeout(
          `https://hiring.amazon.ca/application/api/interview/available-slots/${applicationId}`,
          {
            method: "GET",
            headers,
          }, 3000
        ...
        if (slotsResponse.ok) {
          ...
          const availableSlots = slotsData?.data?.availableSlots || slotsData?.availableSlots || [];

          if (availableSlots.length > 0) {
            // Pick the first available slot (earliest)
            const selectedSlot = availableSlots[0];
            console.log(`📅 Found available slot: ${selectedSlot.start} - ${selectedSlot.end}`);

            // Book the slot
            const bookingResponse = await fetchWithTimeout(
              `https://hiring.amazon.ca/application/api/interview/book-slot`,
              {
                method: "POST",
                headers,
                body: JSON.stringify({
            ...
            if (bookingResponse.ok) {
              ...
              console.log(`✅ Successfully booked interview slot:`, bookingResult);
              return bookingResult;
            }
          }
        }

        // 2. Alternative API endpoint for getting slots
        const altSlotsResponse = await fetchWithTimeout(
          `https://hiring.amazon.ca/application/api/candidate-application/interview-slots/${applicationId}`,
          {
            method: "GET",
            headers,
          }, 3000
        ...
        if (altSlotsResponse.ok) {
          const altSlotsData = await altSlotsResponse.json();
          const slots = altSlotsData?.data?.slots || altSlotsData?.slots || [];

          if (slots.length > 0) {
            const firstSlot = slots[0];
            console.log(`📅 Found alternative slot: ${firstSlot.datetime || firstSlot.start}`);

            // Try to book via alternative endpoint
            const altBookingResponse = await fetchWithTimeout(
              `https://hiring.amazon.ca/application/api/candidate-application/select-interview-slot`,
              {
                method: "POST",
                headers,
                body: JSON.stringify({
                  applicationId,
                  candidateId,
                  slotId: firstSlot.id || firstSlot.slotId,
                  datetime: firstSlot.datetime || firstSlot.start,
                  timezone: "America/Toronto"
                })
              }, 3000
            );

            if (altBookingResponse.ok) {
              const altBookingResult = await altBookingResponse.json();
              console.log(`✅ Successfully booked via alternative endpoint:`, altBookingResult);
              return altBookingResult;
            }
          }
        }

        // 2. Fallback: Try via WebSocket message
        const interviewMsg = attemptScheduleInterviewMessage(applicationId, candidateId, jobId, scheduleId, []);
        socket.send(JSON.stringify(interviewMsg));
        console.log("📤 Sent fallback scheduleInterview via WebSocket");

        // 3. Try direct API call to schedule interview
        const directScheduleResponse = await fetchWithTimeout(
          `https://hiring.amazon.ca/application/api/candidate-application/schedule-interview`,
          {
            method: "POST",
            headers,
            body: JSON.stringify({
        ...
        if (directScheduleResponse.ok) {
          const result = await directScheduleResponse.json();
          console.log(`✅ Direct interview scheduling successful:`, result);
          return result;
        }

        console.log(`⚠️  Could not auto-schedule interview for ${applicationId} - will try again later`);
        return null;

      } catch (error) {
        console.error(`❌ Error auto-scheduling interview for ${applicationId}:`, error);
        return null;
      }
    }
    ...
    socket.onmessage = (event) => {
      try {
        ...
        // Auto-schedule interview when workflow progresses to general-questions or any interview-related step
        if (messageData.stepName === "general-questions") {
          console.log("🎯 Workflow reached general-questions - triggering auto-interview scheduling");
          setTimeout(() => {
            autoScheduleInterview(applicationId, candidateId, jobId, scheduleId);
          }, 1000); // Small delay to let workflow settle
        }
        ...
        if (messageData.stepName === "interview-scheduling" || messageData.stepName === "schedule-interview" || messageData.stepName === "interview-schedule") {
          ...
          // Immediately trigger auto-scheduling
          autoScheduleInterview(applicationId, candidateId, jobId, scheduleId);
        ...
        // Enhanced: Auto-schedule on any workflow step that might lead to interviews
        if (messageData.stepName &&
          (messageData.stepName.includes("interview") ||
            messageData.stepName.includes("schedule") ||
            messageData.stepName === "background-check" ||
            messageData.stepName === "eligibility-check")) {
          console.log(`🎯 Detected interview-related step '${messageData.stepName}' - triggering auto-schedule`);
          setTimeout(() => {
            autoScheduleInterview(applicationId, candidateId, jobId, scheduleId);
          }, 2000);
        }
    ...
    // 🎯 PERIODIC INTERVIEW SCHEDULING ATTEMPTS
    // Try scheduling interview every 10 seconds for the first 2 minutes
    let interviewScheduleAttempts = 0;
    const maxInterviewAttempts = 12; // 12 attempts over 2 minutes

    const interviewScheduleInterval = setInterval(() => {
      if (interviewScheduleAttempts >= maxInterviewAttempts || wsClosed) {
        clearInterval(interviewScheduleInterval);
        console.log(`⏰ Stopped interview scheduling attempts for ${applicationId} after ${interviewScheduleAttempts} attempts`);
        return;
      }

      interviewScheduleAttempts++;
      console.log(`🔄 Interview scheduling attempt #${interviewScheduleAttempts} for ${applicationId}`);
      autoScheduleInterview(applicationId, candidateId, jobId, scheduleId);
    ...
    setTimeout(() => {
      try {
        clearInterval(interviewScheduleInterval); // Clean up the interval
